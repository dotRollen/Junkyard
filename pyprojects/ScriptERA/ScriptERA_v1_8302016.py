...
site = raw_input("Please enter the site name: ")
prime = raw_input("Please enter prime name: ")
MFG = raw_input("Please enter manufacturer name: ")
rackrange = raw_input("Please enter rack range: ")
...
USERID = "root"
PASSWD = "calvin"
#  Model dependant configuration
XML_LIST1 = "R730xd_config_V6.xml"
XML_LIST2 = "R630_config_V3.xml"
#xCAT node list
node_list = subprocess.check_output(["nodels", xcatgroup]).split()
MODEL1 = "R730xd"
MODEL2 = "R630"
DM = "PowerEdge"
...
ws1 = wb.active
ws1.title = location
dest_filename = project + '_' + location + '_' + rackrange + '_EDI_' + timestr + '.xlsx'
...
for node in node_list:
	...
	if ping(node):
		...
		try:
			...
			i = i + 1
			#z = z + 1
			wb.save(filename = dest_filename) # End spread sheet
			...
			if re.findall(MODEL1,rawmodel):
				...
				continue
			print("Starting automated configuration...for " + node)
			...
			print(node + " configuration sent to LifeCycle Controller.")
